...
class VanilaMCTS(object):
    ...
    def selection(self):
        ...
        while not leaf_node_found:
            ...
            if n_child == 0:
                ...
                maximum_uct_value = -100.0 #find max uct value
    ...
    def expansion(self, leaf_node_id):
        ...
        '''
        leaf_state = self.tree[leaf_node_id]['state'] # check the game board of the node
        winner = self._is_terminal(leaf_state) # if player can't do any movement 
        possible_actions = self._get_valid_actions(leaf_state) # [[(row,col), action_idx]] action_idx:1, 2, 3...
        ...
        if winner is None:
            ...
            for action_set in possible_actions:
                ...
                if current_player == 'o':
                    next_turn = 'x'
                    state[action] = 1 # set(row, col) to 1 as o
                else:
                    next_turn = 'o'
                    state[action] = -1 # set(row, col) to -1 as x
            ...
            # print(rand_idx)
            # print('childs: ', childs)
            child_node_id = childs[rand_idx[0]] # random get a child after expansion 
    ...
    def _is_terminal(self, leaf_state):
        ...
        win_mark = self.win_mark # what is win mark
        n_rows_board = len(self.tree[(0,)]['state']) # the current state
        window_size = win_mark # the size of the game board
    ...
    def solve(self):
        for i in range(self.n_iterations):
            leaf_node_id, depth_searched = self.selection() # select the max uct node 
